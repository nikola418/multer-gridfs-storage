...
class GridFsStorage extends node_events_1.EventEmitter {
    ...
    _removeFile(request, file, cb) {
        ...
        bucket.delete(file.id).catch((err) => cb(err));
    }
    /**
    ...
    fromMulterStream(readStream, request, file) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const streamOptions = yield GridFsStorage._mergeProps({ metadata }, settings);
            return new Promise((resolve, reject) => {
    ...
    _updateConnectionStatus() {
        ...
        if (this.client) {
            this.connected = this.db.command({ ping: 1 }) ? true : false;
            return;
        }
        this.connected = this.db.command({ ping: 1 }) ? true : false;
    }
